...
Number of threads have been limited to 20 (which can be increased by changing the size of t and the if statement).
Even for functions that don't require an input, like traversing please give an input of the form 4 0.
Hence, an example run would be:
For inserting 1, deleting 1, traversing and then exiting:
4
1 1 2 1 4 0 5 0
*/
...
int main()
{
    ...
    printf("Creation of B tree for M=%d\n",M);
            while (1) {
            printf("How many threads would you like to make? ");
            scanf("%d", &size);
            if(size > 20){
                printf("Too many threads. Edit source code to increase size or try again with <= 20\n");
                continue;
            }
            int *input;
            input = malloc(2 * size * sizeof(int));
            printf("1. Insertion\t2. Deletion\n");
            printf("3. Searching\t4. Traversal\n");
            printf("5. Exit\t6. Enumerate\n");
            printf("7. Total Keys\t8. Min and Max Keys\n");
            printf("9. Max Level\n");
            ...
            for(int i = 0; i < 2 * size; i = i + 2){
                switch(input[i]){
                    case 1:
                            pthread_create(&t[t_no], NULL, insert, (void *)input[i+1]);
                            break;
                    case 2: 
                            pthread_create(&t[t_no], NULL, DelNode, (void *)input[i+1]);
                            break;
                    case 3:
                            pthread_mutex_lock(&lock);
                            pthread_create(&t[t_no], NULL, search, (void *)input[i+1]);
